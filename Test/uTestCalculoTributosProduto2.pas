...
type
  ...
  public
    ...
    procedure TestCalcularTributosProduto_ValorZero;
     /// <summary>
    /// Método de teste para NCM inválido.
    /// </summary>
...
begin
  ...
  Check(Resultado.CodigoProduto = 1, 'Código do produto incorreto.');
  ...
  Check(Assigned(Resultado), 'O resultado não deve ser nulo.');
  Check(VarIsEmpty(Resultado.Mensagem), 'A mensagem deve estar vazia em caso de sucesso.');
  Check(Resultado.CodigoProduto = 2, 'Código do produto incorreto.');
...
var
  ...
  Produto: TProduto;
begin
   // Altera o NCM do produto 1 para um valor inválido
  ...
  Check(Assigned(Resultado), 'O resultado não deve ser nulo.');
  Check(not VarIsEmpty(Resultado.Mensagem), 'A mensagem não deve estar vazia.');
  Check(Resultado.Mensagem = 'NCM do produto inválido: ABCDEFGH', 'Mensagem de erro incorreta para NCM inválido.');
  Resultado.Free;

  // Restaura o NCM para um valor válido para não afetar outros testes
  Produto := FRepositorioProduto.BuscarPorCodigo(1);
  Produto.Ncm := '12345678';
  FRepositorioProduto.Atualizar(Produto);
end;
